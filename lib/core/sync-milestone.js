...
// Load configuration
const CONFIG_PATH = path.join(process.cwd(), '.claude', 'config.json');
let config = {};
...
const CLAUDE_PATH = path.join(process.cwd(), config.paths?.mainDoc || 'README.md');
...
// Update main documentation with latest milestone info
function updateClaudeMd(status) {
  if (!fs.existsSync(CLAUDE_PATH)) {
    console.error('❌ Main documentation not found:', CLAUDE_PATH);
    return false;
  }
...
function generateBugReport() {
  ...
  try {
    const testResult = require('child_process').execSync(
      'npm test --silent 2>&1 | grep -E "FAIL|failed" | wc -l',
      { encoding: 'utf8' }
    ).trim();
    ...
    if (parseInt(testResult) > 0) {
      bugs.push({
        ...
        action: 'Run npm test to see failures'
      });
    }
  } catch (error) {
  ...
  try {
    ...
    if (parseInt(tsErrors) > 0) {
      bugs.push({
        ...
        action: 'Run npm run type-check for details'
      });
    }
  } catch (error) {
    // No TS errors
...
function syncMilestone() {
  ...
  if (updateClaudeMd(status)) {
    console.log('✅ Updated main documentation');
  }
